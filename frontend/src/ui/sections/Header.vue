...
function onCinemaChange(cinemaId: number) {
  const cinema = cinemaStore.cinemas.find((c) => c.id === cinemaId)

  if (cinema) {
    themeStore.setPrimaryColor(cinema.color)
    cinemaStore.setActiveCinema(cinema)
  }
}
</script>
...
<template>
  <header class="bg-primary-100 relative flex items-center justify-between rounded-lg p-4 shadow-md">
    <div>
      ...
      <select v-model="selectedCinemaId" class="bg-primary-100 border-primary-400 rounded-lg border p-2" @change="onCinemaChange(selectedCinemaId!)">
        <option value="" disabled selected>Select cinema</option>
        <option v-for="cinema in cinemaStore.cinemas" :key="cinema.id" :value="cinema.id">
          {{ cinema.name }}
        </option>
      </select>
    </div>
